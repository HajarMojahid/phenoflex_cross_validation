...
check_temperature_record(temps_DE)
...
# Merge all pheno
pheno_all <- bind_rows(bind_rows(cal_data_DE, .id = "Cultivar"), bind_rows(val_data_DE, .id = "Cultivar"))

# Select only relevant columns
pheno_all <- select(pheno_all, Cultivar, Year, Bloom_full_JDay) %>% rename("pheno" = "Bloom_full_JDay")
...
for (i in 1 : length(calibration_list)) {
  ...
  SeasonList[[i]] <- genSeasonList(hourtemps_DE, mrange = c(8, 6), years = years_cal)
  eval_SeasonList[[i]] <- genSeasonList(hourtemps_DE, mrange = c(8, 6), years = years_val)
}
...
par_01 <-   c(rep(55, n_cult), rep(220, n_cult),  rep(0.5, n_cult), 25, 3372.8,  9900.3, 6319.5, 5.939917e13,  4, 36,  4,  1.60)
...
# Adjust the values
lower_02 <- c(rep(19, n_cult), rep(110, n_cult), rep(0.01, n_cult), 21, 2600.0,  9500.0, 5900.0, 5.5e13, 0, 21,  0,  0.05)
par_02 <-   Fit_res_01$par
upper_02 <- c(rep(81, n_cult), rep(450, n_cult), rep(1.25, n_cult), 29, 4400.0, 10400.0, 6700.0, 6.3e13, 12, 29, 12, 30.00)
...
# Adjust the values
lower_03 <- c(rep(31, n_cult), rep(200, n_cult), rep(0.001, n_cult), 20, 2600.0,  9200.0, 5500.0, 5.4e13, -1.56, 22,  0,  0.05)
par_03 <-   Fit_res_02$par
upper_03 <- c(rep(62, n_cult), rep(265, n_cult), rep(0.650, n_cult), 30, 4000.0, 10600.0, 6700.0, 6.4e13, 10, 35, 10, 15.00)
...
# Adjust the values
lower_04 <- c(rep(36, n_cult), rep(210, n_cult), rep(0.001, n_cult), 20, 2600.0,  9000.0, 5500.0, 5.4e13, -4.5, 22,  0,  0.05)
par_04 <-   Fit_res_03$par
upper_04 <- c(rep(64, n_cult), rep(255, n_cult), rep(0.700, n_cult), 30, 4000.0, 10600.0, 6900.0, 6.4e13,  11, 32, 12, 15.00)
...
# Adjust the values
lower_05 <- c(rep(40, n_cult), rep(200, n_cult), rep(0.001, n_cult), 20, 2600.0,  9000.0, 5500.0, 5.4e13, -5, 25,  0,  0.05)
par_05 <-   Fit_res_04$par
upper_05 <- c(rep(70, n_cult), rep(260, n_cult), rep(0.500, n_cult), 30, 4000.0, 10600.0, 6900.0, 6.4e13, 12, 35, 12, 15.00)
...
write.csv(summary_calibration, "results/03_summary_cal_DE.csv", row.names = FALSE)
write.csv(summary_validation, "results/06_summary_val_DE.csv", row.names = FALSE)
write.csv(summary_params, "results/09_summary_pars_DE.csv", row.names = FALSE)
write.csv(summary_metrics, "results/12_summary_metrics_DE.csv", row.names = FALSE)
