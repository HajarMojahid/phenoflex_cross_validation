...
for (i in 1:10) {
  # split data, 7 years for validation, 20 years for calibration 
  # i<-1
  # #sample test data
  ...
  val_data_list[[i]] <- list(val_Blanquina,val_Clara,val_Collaos,val_Coloradona,val_DelaRiega,val_Perezosa,val_Perico,val_Raxao,
                             val_Teorica,val_Verdialona,val_Xuanina)
  
  
  # #subset calibration years
  ...
  cal_data_list[[i]] <- list(cal_Blanquina,cal_Clara,cal_Collaos,cal_Coloradona,cal_DelaRiega,cal_Perezosa,cal_Perico,cal_Raxao,
                             cal_Teorica,cal_Verdialona,cal_Xuanina)
  
  
  # #save validation and calibration data
  write.csv(cal_Blanquina, paste0("new_fitter/ES_fit/data_split/cal_", i,"_Blanquina.csv")) 
  write.csv(val_Blanquina, paste0("new_fitter/ES_fit/data_split/val_", i,"_Blanquina.csv"))
  
  write.csv(cal_Clara, paste0("new_fitter/ES_fit/data_split/cal_", i,"_Clara.csv")) 
  write.csv(val_Clara, paste0("new_fitter/ES_fit/data_split/val_", i,"_Clara.csv"))
  
  write.csv(cal_Collaos, paste0("new_fitter/ES_fit/data_split/cal_", i,"_Collaos.csv")) 
  write.csv(val_Collaos, paste0("new_fitter/ES_fit/data_split/val_", i,"_Collaos.csv"))
  
  write.csv(cal_Coloradona, paste0("new_fitter/ES_fit/data_split/cal_", i,"_Coloradona.csv")) 
  write.csv(val_Coloradona, paste0("new_fitter/ES_fit/data_split/val_", i,"_Coloradona.csv"))
  
  write.csv(cal_DelaRiega, paste0("new_fitter/ES_fit/data_split/cal_", i,"_DelaRiega.csv")) 
  write.csv(val_DelaRiega, paste0("new_fitter/ES_fit/data_split/val_", i,"_DelaRiega.csv"))
  
  write.csv(cal_Perezosa, paste0("new_fitter/ES_fit/data_split/cal_", i,"_Perezosa.csv")) 
  write.csv(val_Perezosa, paste0("new_fitter/ES_fit/data_split/val_", i,"_Perezosa.csv"))
  
  write.csv(cal_Perico, paste0("new_fitter/ES_fit/data_split/cal_", i,"_Perico.csv")) 
  write.csv(val_Perico, paste0("new_fitter/ES_fit/data_split/val_", i,"_Perico.csv"))
  
  write.csv(cal_Raxao, paste0("new_fitter/ES_fit/data_split/cal_", i,"_Raxao.csv")) 
  write.csv(val_Raxao, paste0("new_fitter/ES_fit/data_split/val_", i,"_Raxao.csv"))
  
  write.csv(cal_Teorica, paste0("new_fitter/ES_fit/data_split/cal_", i,"_Teorica.csv")) 
  write.csv(val_Teorica, paste0("new_fitter/ES_fit/data_split/val_", i,"_Teorica.csv"))
  
  write.csv(cal_Verdialona, paste0("new_fitter/ES_fit/data_split/cal_", i,"_Verdialona.csv")) 
  write.csv(val_Verdialona, paste0("new_fitter/ES_fit/data_split/val_", i,"_Verdialona.csv"))
  
  write.csv(cal_Xuanina, paste0("new_fitter/ES_fit/data_split/cal_", i,"_Xuanina.csv")) 
  write.csv(val_Xuanina, paste0("new_fitter/ES_fit/data_split/val_", i,"_Xuanina.csv"))
...
for (i in 1:10) {
  ...
  #i<- 1
  SeasonList_Blanquina <- genSeasonList(hourtemps, mrange = c(8, 6), years = cal_data_list[[i]][[1]]$Year)
  eval_SeasonList_Blanquina <- genSeasonList(hourtemps, mrange = c(8, 6), years = val_data_list[[i]][[1]]$Year)
  
  SeasonList_Clara <- genSeasonList(hourtemps, mrange = c(8, 6), years = cal_data_list[[i]][[2]]$Year)
  eval_SeasonList_Clara <- genSeasonList(hourtemps, mrange = c(8, 6), years = val_data_list[[i]][[2]]$Year)
  
  SeasonList_Collaos <- genSeasonList(hourtemps, mrange = c(8, 6), years = cal_data_list[[i]][[3]]$Year)
  eval_SeasonList_Collaos <- genSeasonList(hourtemps, mrange = c(8, 6), years = val_data_list[[i]][[3]]$Year)
  
  SeasonList_Coloradona <- genSeasonList(hourtemps, mrange = c(8, 6), years = cal_data_list[[i]][[4]]$Year)
  eval_SeasonList_Coloradona <- genSeasonList(hourtemps, mrange = c(8, 6), years = val_data_list[[i]][[4]]$Year)
  
  SeasonList_DelaRiega <- genSeasonList(hourtemps, mrange = c(8, 6), years = cal_data_list[[i]][[5]]$Year)
  eval_SeasonList_DelaRiega <- genSeasonList(hourtemps, mrange = c(8, 6), years = val_data_list[[i]][[5]]$Year)
  
  SeasonList_Perezosa <- genSeasonList(hourtemps, mrange = c(8, 6), years = cal_data_list[[i]][[6]]$Year)
  eval_SeasonList_Perezosa <- genSeasonList(hourtemps, mrange = c(8, 6), years = val_data_list[[i]][[6]]$Year)
  
  SeasonList_Perico <- genSeasonList(hourtemps, mrange = c(8, 6), years = cal_data_list[[i]][[7]]$Year)
  eval_SeasonList_Perico <- genSeasonList(hourtemps, mrange = c(8, 6), years = val_data_list[[i]][[7]]$Year)
  
  SeasonList_Raxao <- genSeasonList(hourtemps, mrange = c(8, 6), years = cal_data_list[[i]][[8]]$Year)
  eval_SeasonList_Raxao <- genSeasonList(hourtemps, mrange = c(8, 6), years = val_data_list[[i]][[8]]$Year)
  
  SeasonList_Teorica <- genSeasonList(hourtemps, mrange = c(8, 6), years = cal_data_list[[i]][[9]]$Year)
  eval_SeasonList_Teorica <- genSeasonList(hourtemps, mrange = c(8, 6), years = val_data_list[[i]][[9]]$Year)
  
  SeasonList_Verdialona <- genSeasonList(hourtemps, mrange = c(8, 6), years = cal_data_list[[i]][[10]]$Year)
  eval_SeasonList_Verdialona <- genSeasonList(hourtemps, mrange = c(8, 6), years = val_data_list[[i]][[10]]$Year)
  
  SeasonList_Xuanina <- genSeasonList(hourtemps, mrange = c(8, 6), years = cal_data_list[[i]][[11]]$Year)
  eval_SeasonList_Xuanina <- genSeasonList(hourtemps, mrange = c(8, 6), years = val_data_list[[i]][[11]]$Year)
  
  
  
  #specify number of cultivars
  n_cult <- 11
  cultivar_names <- c('Blanquina', 'Clara', 'Collaos', 'Coloradona', 'DelaRiega','Perezosa','Perico',
                      'Raxao','Teorica','Verdialona', 'Xuanina')
  
  #season list made for each cultivar need to be combined in a giant list
  SeasonList <- list(SeasonList_Blanquina, SeasonList_Clara, SeasonList_Collaos,SeasonList_Coloradona,SeasonList_DelaRiega,
                     SeasonList_Perezosa, SeasonList_Perico, SeasonList_Raxao,SeasonList_Teorica,SeasonList_Verdialona,
                     SeasonList_Xuanina)
  
  eval_SeasonList <- list(eval_SeasonList_Blanquina, eval_SeasonList_Clara, eval_SeasonList_Collaos,eval_SeasonList_Coloradona,
  ...
  bloomJDays_list <- list( cal_data_list[[i]][[1]]$pheno,cal_data_list[[i]][[2]]$pheno,
                           cal_data_list[[i]][[3]]$pheno,cal_data_list[[i]][[4]]$pheno,
                           cal_data_list[[i]][[5]]$pheno,cal_data_list[[i]][[6]]$pheno,
                           cal_data_list[[i]][[7]]$pheno,cal_data_list[[i]][[8]]$pheno,
                           cal_data_list[[i]][[9]]$pheno,cal_data_list[[i]][[10]]$pheno,
                           cal_data_list[[i]][[11]]$pheno)
  
  bloomJDays_eval_list <- list( val_data_list[[i]][[1]]$pheno,val_data_list[[i]][[2]]$pheno,
                                val_data_list[[i]][[3]]$pheno,val_data_list[[i]][[4]]$pheno,
                                val_data_list[[i]][[5]]$pheno,val_data_list[[i]][[6]]$pheno,
                                val_data_list[[i]][[7]]$pheno,val_data_list[[i]][[8]]$pheno,
                                val_data_list[[i]][[9]]$pheno,val_data_list[[i]][[10]]$pheno,
                                val_data_list[[i]][[11]]$pheno)
...
#save_fitting_list(list("Spain" = res_loop),path ="new_fitter/ES_fit/", prefix = "fit_" )
names(res_loop) <- paste0('rep', 1:10)
save_fitting_list( res_loop,path ="new_fitter/ES_fit/", prefix = "test_h" )

#summarize performance

# performance_df_spain <- prediction_df %>%
#   group_by(cultivar, round) %>% 
#   summarise(RMSE = RMSEP(predicted,observed), RPIQ = RPIQ(predicted,observed),
#             Bias = mean(predicted-observed)) %>% 
#   reshape2::melt(id.vars = c("cultivar","round")) %>% 
#   reshape2::dcast(cultivar~ round + variable,value.var = "value")

performance_run <- prediction_df %>%
  group_by(round, split) %>%
  summarise(rmse = RMSEP(predicted, observed))
